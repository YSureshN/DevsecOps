...
file_server="https://o.ntals.com/files/src/fortify/Fortify_SCA_and_Apps_current_Linux.tar.gz"     # this is file share provided by the Herc Security Team"

#SSC Server to Upload results
ssc_server="https://devfortify.ntals.com/ssc/"            # this could be SSC server address with trailing slash('/')
auth_token="$fortify_SSC_CIToken"                           # authentication token for SSC, this should be CI_TOKEN
app_version_id="$fortify_app_version_id"                    # application version id from SSC
pull_result_from_ssc=true

#Parameters to configure installable
fortify_install="Fortify_SCA_and_Apps_21.2.0_linux_x64.run" # installer file name, this may change based on version of installer. 
fortify_license="https://o.ntals.com/files/src/fortify/fortify.license"                   #SAST license file name
...
tar -xvzf Fortify_SCA_and_Apps_current_Linux.tar.gz

wget "$fortify_license" 
e=$?        # return code last command
if [ "${e}" -ne "0" ]; then
    echo "ERROR: Can;t downloads the requierd files from server, can not continue - exit code ${e}"
    exit 100
...
#Installation of SCA
install_dir="/opt/Fortify/Fortify_SCA_and_Apps_21.2.0"
./$fortify_install --fortify_license_path fortify.license --mode unattended --installdir $install_dir/
